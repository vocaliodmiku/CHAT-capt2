...
def has_repeated_phrases(text, threshold=5, ratio_threshold=0.2):
    """
    判断是否存在大量重复话语
    """
    ...
    # 判断是否存在大量重复话语
    repeated_word_ratio = len(repeated_words) / len(words)
    if repeated_word_ratio > ratio_threshold:
        return True
    else:
        return False
...
class RLTrainer(TrainerBase):  # pylint: disable=too-many-instance-attributes
    ...
    def rollout(self, prompt_only_batch: PromptOnlyBatch) -> list[dict[str, Any]]:
        ...
        if self.args.print_generated:
            if is_main_process():
                self.logger.print('Generated sequences:')
                # self.logger.print(sequences.shape)
                # self.logger.print(input_ids.shape)
                for seq in sequences:
                    self.logger.print(self.tokenizer.decode(seq, skip_special_tokens=True))
        if self.args.truncate_sequences:
            prompt_length = input_ids.shape[1]
            valid_ans_len = (sequences[:, prompt_length:] != self.tokenizer.pad_token_id).sum(dim=-1)
            val_index = []
            for index in range(input_ids.shape[0]):
                if valid_ans_len[index] < 10 or valid_ans_len[index] > 1024:
                    if is_main_process():
                        self.logger.print('deleted Generated sequences because length')
                        self.logger.print(self.tokenizer.decode(sequences[index], skip_special_tokens=True))
                    continue
                elif has_repeated_phrases(self.tokenizer.decode(sequences[index], skip_special_tokens=True)):
                    if is_main_process():
                        self.logger.print('deleted Generated sequences because duplicate:')
                        self.logger.print(self.tokenizer.decode(sequences[index], skip_special_tokens=True))
                    continue
                else:
            ...
            # if is_main_process():
            #     self.logger.print(sequences.shape)
            sequences = sequences[val_index, :]
            input_ids = input_ids[val_index, :]
            # if is_main_process():
            #     self.logger.print(sequences.shape)
            #     self.logger.print(input_ids.shape)
            #     self.logger.print(sequences)
            #     self.logger.print(input_ids)
